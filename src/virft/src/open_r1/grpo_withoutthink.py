...
# import sys
# sys.path.append("/inspire/hdd/global_user/zhouwei-240108540164/autodrive/Visual-RFT-main/src/virft/src")
...
def main(script_args, training_args, model_args):
    ...
    # dataset = DatasetDict.load_from_disk(script_args.dataset_name) #作者在issue里面改的
    dataset = load_dataset(script_args.dataset_name, name=script_args.dataset_config)
    ...
    def make_conversation_image(example):
        ...
        core_problem = example["problem"].split("places).")[0] + "places)."
    
        # 步骤2：添加去掉思考过程后的格式要求（只保留answer标签）
        new_problem = core_problem + """
        If no object belonging to the category 'surfboard' in the image, return 'No Objects'.
