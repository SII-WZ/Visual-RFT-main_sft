...
#from transformers.generation import GenerationConfig 原文是这个
...
def extract_bbox(response):
    ...
    if start_tag in input_str:
        ...
        print(content_str_corrected ,"测一下extar2")
...
model_path = "./out/Qwen2-VL-2B-Instruct_GRPO_coco_base65cate_6k/checkpoint-1500"
#ori_processor_path = "./Qwen/Qwen2-VL-2B-Instruct"
#model_path = "./Qwen/Qwen2-VL-2B-Instruct"
ori_processor_path=  "./out/Qwen2-VL-2B-Instruct_GRPO_coco_base65cate_6k/checkpoint-1500"

model_path = "./out/Qwen2-VL-7B-Instruct_GRPO_coco_base65cate_6k/checkpoint-1500"
...
def run(rank, world_size):
    ...
    with open('./share_data/coco_val/annotations/instances_val2017.json', 'r') as json_file:
    ...
    pred_results = []
    if '2B' in model_path:
        exist_cat = json.load(open("./coco_evaluation/exist_map_coco_Qwen2_vl_2B_baseline.json", 'r'))
    elif '7B' in model_path:
        exist_cat = json.load(open("./coco_evaluation/exist_map_coco_Qwen2_vl_7B_baseline.json", 'r'))
    testnum=0
    for image in tqdm(split_images): 
        image_id = image['id']
        image_height = image['height']
        image_width = image['width']
        image_path = './share_data/coco_val/val2017/'+image['file_name']    ### Modify according to your own image path.
        print(image_path,"这里是路径")
        ...
        for cate in exist_cat[str(image_id)]:
          ...
          #  print(cate,"这是cate")
            ...
            # )
            # question = (
            #     f"Detect all objects belonging to the category '{category}' in the image, and provide the bounding boxes format'Position': [x1, y1, x2, y2](x,y: between 0 and 1000, integer) and confidence format'Confidence': number (number: between 0 and 1, with two decimal places).\n"
            #     f"If no object belonging to the category '{category}' in the image, return 'No Objects'.\n"
            #     "Output the thinking process in <think>  ...  </think> and final answer in <answer> [{'Position': [x1, y1, x2, y2], 'Confidence': number}] </answer> tags."
            #     "The output answer format should be as follows:\n"
            #     "<think> ... </think> <answer>[{'Position': [x1, y1, x2, y2], 'Confidence': number}]</answer>\n"
            #     "Please strictly follow the format."
            # )
            ...
            #     f"Detect all objects belonging to the category '{category}' in the image, and provide the bounding boxes format'Position': [x1, y1, x2, y2](x,y: between 0 and 1000, integer) and confidence format'Confidence': number (number: between 0 and 1, with two decimal places).\n"
            #     f"If no object belonging to the category '{category}' in the image, return 'No Objects'.\n"
            #     "Output the final answer in <answer> [{'Position': [x1, y1, x2, y2], 'Confidence': number}] </answer> tags."
            #     "The output answer format should be as follows:\n"
...
def main():
    ...
    if multiprocess:
        ...
        ### save path
        with open('./coco_evaluation/2b_sft_prediction_results.json', 'w') as json_file:
