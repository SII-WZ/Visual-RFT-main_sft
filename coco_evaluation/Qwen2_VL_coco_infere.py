...
def trans_bbox(image_height, image_width, position):
    # ========== 安全防护：如果不是4个值，直接跳过，返回空框 ==========
    if not isinstance(position, (list, tuple)) or len(position) != 4:
        print(f"[警告] 异常坐标 {position}，已跳过！")
        return [0, 0, 0, 0]  # 返回合法空框，程序不会崩溃
...
def remove_duplicates(bbox_list):
    ...
    for bbox in bbox_list:
        ...
        if 'Position' not in bbox:
            continue  # 缺少 Position，直接跳过这个框 我自己加的
...
model_path = "./out/Qwen2-VL-2B-Instruct_SFT_coco_base65cate_6k_sft_wothink/checkpoint-1500"
ori_processor_path=  "./Qwen/Qwen2-VL-2B-Instruct"
#ori_processor_path = "./Qwen/Qwen2-VL-2B-Instruct"
#model_path = "./Qwen/Qwen2-VL-2B-Instruct"


# model_path = "./out/Qwen2-VL-7B-Instruct_GRPO_coco_base65cate_6k/checkpoint-1250"
# model_path = "./Qwen/Qwen2-VL-7B-Instruct"
# ori_processor_path = "./Qwen/Qwen2-VL-7B-Instruct"
# ori_processor_path=  "./out/Qwen2-VL-7B-Instruct_GRPO_coco_base65cate_6k/checkpoint-1250"
    ...
    for image in tqdm(split_images): 
        ...
        for cate in exist_cat[str(image_id)]:
          ...
          #  print(cate,"这是cate")
            ...
            ### open vocabulary experiment:  15 new classes 下面注销就80类全选
         ...
         ################原版question
            ...
            try:
               ...
               # print("怎么进不去吗2")
                ...
                #generated_ids = model.generate(**inputs, max_new_tokens=256) #原来的
                generated_ids = model.generate(
                **inputs,
                max_new_tokens=256,
                do_sample=False,  #自己加的关闭采样
                temperature=0,          # 极低温度，接近贪心解码（避免随机）
              ...
              #  print(response,"看看response3")
                ...
                try:
                    ...
                    if bbox_list==None or type(bbox_list[0])==str:
                        ...
                        for bbox in bbox_list:
                            if 'Position' not in bbox or 'Confidence' not in bbox:
                                continue
...
def main():
    ...
    if multiprocess:
        ...
        ### save path
        print("save_path")
        with open('./coco_evaluation/2b_sft_prediction_results_80_wothink_new.json', 'w') as json_file:
