...
#     "../out/lisa_train_GIoU/checkpoint-450", torch_dtype=torch.bfloat16, device_map="auto", attn_implementation="flash_attention_2"
# ).eval()

# processor = AutoProcessor.from_pretrained("../out/lisa_train_GIoU/checkpoint-450")
...
# model = Qwen2VLForConditionalGeneration.from_pretrained(
#     "../Zery/Qwen2-VL-7B_visual_rft_lisa_IoU_reward", torch_dtype=torch.bfloat16, device_map="auto",local_files_only=True
# ).eval()

# processor = AutoProcessor.from_pretrained("../Zery/Qwen2-VL-7B_visual_rft_lisa_IoU_reward",local_files_only=True)

# model = Qwen2VLForConditionalGeneration.from_pretrained(
#     "Qwen/Qwen2-VL-2B-Instruct", device_map="auto",local_files_only=True
# ).eval()
...
def evaluate_model(tasks,save_name="val" ,split=0, split_num=1):  # 接收参数
    results = []
    box_res =[]
    for task in tasks:
        ...
        for item in tqdm(screenspot_data):
            ...
            try:
                ...
                box_res.append(
                    ...
                    "thinking_process": f"Error: {str(e)}"+response,
                    "status": "failed"
                    }
                )
                ious.append(0)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--task', type=str, default="test", help='任务名称')
    parser.add_argument('--split', type=int, default=0, help='当前分片索引（从0开始）')
    parser.add_argument('--split_num', type=int, default=1, help='总分片数')
    parser.add_argument('--save_name', type=str, default="7b_ori'_test", help='保存的文件夹名字')
    args = parser.parse_args()
    ...
    # tasks = ["test_error"]
    results = evaluate_model(tasks, save_name=save_name,split=split, split_num=split_num)
