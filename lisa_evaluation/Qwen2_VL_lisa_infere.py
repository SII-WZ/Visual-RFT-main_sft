...
def evaluate_model(tasks):
    ...
    for task in tasks:
        ...
        ious = []
        screenspot_data = json.load(open(f"../share_data/lisa_{task}.json", 'r'))       #json.load(open(f"path/to/lisa_{task}.json", 'r'))
        ...
        print(f"SPLIT={os.environ['SPLIT']}有 {len(screenspot_data)} 条数据")#我查看的
